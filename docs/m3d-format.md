...
Render-flag naming follows the source's `_` plus base-36 character convention: bit 0 is translucency, bit 1 animated UV, bit 2 transparency and bit 4 colour-key. The preview combines group and model filename flags, uses BMP palette entry zero for indexed foliage cutouts, sorts transparent batches, and scrolls UVs for bit 1. Water M3X groups are combined static layers, not animation frames. **Confidence 1** for exact opacity and UV-scroll speed; simple directional shading remains a preview choice. BMP row order is supplied by GdkPixbuf and UV V is used as stored.
