...
int main(){try {
    ...
    auto charge=cmds(),walk=cmds();check(charge.command(neo::UnitCommand::Charge) && charge.units[0].target==1 && charge.units[0].moving,"Charge button acquires a nearby enemy");walk.order({30,0,0});charge.tick();walk.tick();check(charge.units[0].position.x>walk.units[0].position.x,"Charge movement responds to button command");
    auto spell=cmds();spell.units[0].regiment.wizard=1;spell.magic_power=1;auto spell_target=spell.units[1].regiment.alive;check(spell.cast_magic() && spell.magic_power==0 && spell.projectiles.size()==1 && spell.projectiles[0].magic,"Magic button spends power and launches an arcane bolt");for(unsigned i=0;i<60 && !spell.projectiles.empty();++i)spell.tick();check(spell.units[1].regiment.alive+3>=spell_target,"Arcane bolt resolves after flight");
