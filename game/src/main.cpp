...
void inspect(const std::filesystem::path& path) {
    ...
    else if(ext==".prj") {auto d=prj::Document::decode(bytes);auto catalog=d.catalog();std::vector<unsigned> count(catalog.size());for(unsigned i=0;i<d.instance_count();++i){auto slot=d.field(i,0x40);if(slot && slot<=count.size())++count[slot-1];}std::cout<<d.width()<<"x"<<d.height()<<", "<<d.instance_count()<<" furniture instances\n";for(size_t i=0;i<catalog.size();++i)if(count[i])std::cout<<count[i]<<" x "<<catalog[i]<<"\n";}
    else if(ext==".m3d" || ext==".m3x"){auto model=m3d::Model::decode(bytes);std::cout<<model.triangles<<" triangles, bounds "<<model.minimum.x<<","<<model.minimum.y<<","<<model.minimum.z<<" to "<<model.maximum.x<<","<<model.maximum.y<<","<<model.maximum.z<<"\n";for(size_t i=0;i<model.textures.size();++i)std::cout<<i<<" "<<model.textures[i]<<"\n";for(const auto& batch:model.batches){m3d::Vec3 center{};for(const auto& vertex:batch.vertices){center.x+=vertex.position.x;center.y+=vertex.position.y;center.z+=vertex.position.z;}float n=float(std::max<size_t>(1,batch.vertices.size()));std::cout<<"batch "<<batch.material<<" center "<<center.x/n<<","<<center.y/n<<","<<center.z/n<<"\n";}}
