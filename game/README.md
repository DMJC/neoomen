...
The campaign interpreter reads the original embedded script from `PRG_ENG/DarkOmen.exe`; it never executes native Windows code. Choices select original branches, missions load the active roster, and battle results carry casualties into subsequent script execution. Gold, roster activation, objectives, script stacks and choices survive native checkpoints. F5 saves to `neoomen.save` (override with `--save`); F9 restores. A checkpoint taken during a battle **restarts that mission from its campaign roster**. It does not preserve battle positions, CTL state or support original SAV files.
...
Water meshes named by the PRJ WATR section now animate their vertices with continuous world-space waves, matching surface normals and flowing texture coordinates. Animation runs during deployment and battle and freezes while paused. Shared patch vertices use identical phases to prevent cracks. Wave amplitude, speed and UV flow are provisional visual parameters; no original vertex-animation sequence has been recovered.
